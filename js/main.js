...
if (contactForm) {
  contactForm.addEventListener("submit", async (e) => {
    ...
    /* Log contact form submission to the admin activity feed.
       Sends the full message + email to the Netlify Function so
       the admin panel sees it on every device, and the visitor
       receives an email receipt.                                */
    const subjectField = contactForm.querySelector('select');
    const subjectValue = subjectField ? subjectField.value : '';
    const fullMessage  = message.value.trim();
    const fullName     = `${firstName.value.trim()} ${lastName.value.trim()}`;

    /* If the visitor mentions certification, route the event
       type so it shows under the "Certification" filter.       */
    const isCertRequest = /certif/i.test(subjectValue) || /certif/i.test(fullMessage);
    const eventType     = isCertRequest ? 'certification_request' : 'contact_form';
    ...
    if (typeof logActivity === 'function') {
      logActivity(eventType, {
        ...
        message: fullMessage,
        details: `${subjectValue} — ${fullMessage.substring(0, 80)}${fullMessage.length > 80 ? '\u2026' : ''}`
      });
    }
